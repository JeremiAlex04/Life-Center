...
public interface CitaRepository extends JpaRepository<Cita, Long> {
    ...
    @Query("SELECT CASE WHEN COUNT(c) > 0 THEN true ELSE false END FROM Cita c WHERE c.medico.idMedico = :medicoId AND c.fecha = :fecha AND c.hora = :hora")
    boolean existeCitaMedico(@Param("medicoId") Long medicoId, @Param("fecha") LocalDate fecha,
